...
function getUserId() {
    let id = localStorage.getItem('emoji_chat_user_id');
    if (!id) {
        id = crypto.randomUUID();
        localStorage.setItem('emoji_chat_user_id', id);
    }
    return id;
}
...
function connectWS(roomId) {
    ...
    const url = `${proto}//${location.host}/ws/${roomId}?userId=${userId}`;
...
function showQRModal() {
    const url = `${location.origin}/static/index.html#room/${currentRoomId}`;
